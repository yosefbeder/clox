...
void emitBytes(Compiler *compiler, uint8_t byte1, uint8_t byte2, Token *token)
{
    emitByte(compiler, byte1, token);
    emitByte(compiler, byte2, token);
}
...
{
    emitBytes(compiler, OP_NIL, OP_RETURN, token);
}
...
static void expression(Compiler *compiler, int minBP)
{
    ...
    while (!check(compiler, TOKEN_EOF))
    {
        ...
        if (operator.type == TOKEN_EQUAL)
            errorAt(compiler, &operator, "Bad assignment target");
        ...
        if (opCode == -1)
        {
            ...
            case TOKEN_LEFT_PAREN:
            {
                ...
                emitBytes(compiler, OP_CALL, argsCount, &operator);
                break;
            }
            default:;
            }
...
#define PARSE_PARAM                                                             \
    ...
    for (int i = 0; i < funCompiler.currentUpValue; i++)
    {
        ...
        emitBytes(compiler, (uint8_t)upValue->local, upValue->index, &token);
    }
